...
export function initState (vm: Component) {
  ...
  if (opts.props) initProps(vm, opts.props) // 初始化props,把props的值挂在vm实例上
  if (opts.methods) initMethods(vm, opts.methods) // 初始化methods，把methods中函数挂到vm上
  if (opts.data) {
    initData(vm) // 初始化data, 把data挂在vm上并观测data，此时可以调用props、methods
  } else {
...
function initProps (vm: Component, propsOptions: Object) {
  ...
  if (!isRoot) { // 非根组件的 props 关闭观测
    toggleObserving(false) // 关闭数据监测
  }
  for (const key in propsOptions) {
    keys.push(key) // 把所有prop的key另存在options的 _propKeys中
    const value = validateProp(key, propsOptions, propsData, vm) // 对prop进行验证并取验证后的返回值
    ...
    // instantiation here.
    if (!(key in vm)) {
      proxy(vm, `_props`, key) // 在vue实例上建立一个该值的引用
    }
  }
  toggleObserving(true) // 打开数据监测
}

/**
 * 把data挂在vm上，此时可以调用props、methods
 * @param {Component} vm
 */
...
function initComputed (vm: Component, computed: Object) {
  ...
  for (const key in computed) {
    ...
    if (!(key in vm)) {
      defineComputed(vm, key, userDef) // 给computed设置get和set函数
...
export function defineComputed (
  ...
  if (typeof userDef === 'function') { // computed形式是 test(){} 形式
